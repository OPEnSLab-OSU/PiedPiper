...
void piedPiper::sendRawData() {
  /*/
  Serial.write(255);

  // Send raw audio data
  Serial.write(128 + samplePow);
...
void piedPiper::sendSpectrumData() {
  Serial.write(255);
  Serial.write(samplePow - 1);
...
void piedPiper::playback(int mimic, int clk, int latch, int data) {
  ...
  switch (mimic) { //maps the inputted audio # to the matching shift register #
    ...
    default: mimic = 10;
      ...
      shiftOut(data, clk, MSBFIRST, 0b11111111); //sends data to shift register
  ...
  digitalWrite(latch, LOW); //turns off register
  digitalWrite(clk, LOW); //sets clock low indicating the start of a byte
  shiftOut(data, clk, MSBFIRST, 0b11111110); //sends data to shift register

...
#define sd 0  //define if using SD and RTC
...
#define in3 13
#define PLAYBACK 1
#define FFPLAYBACK_TIME 10    //set how long the system will playback mimc signal(seconds)
...
void setup() {
  ...
  if (p.getDebug() or p.getDebugSignal()) { //turns on built in neopixel for visual debugging
    indicator.begin();
    LED_set = 1;
    indicator.setPixelColor(0, 255, 64, 128);
    indicator.show();
  }
...
void LiveSpectrogram() {
  while (Serial.available()) {
    switch (Serial.read()) {
      case 0: datType = false;
      case 1: datType = true;
      case 2: normalize = !normalize;
      default: break;
    }
  }
...
void loop() {
  ...
  switch (mode()) {
    ...
    case 3: playbackTime(13, 6000, 0, 1); break; //only playback
